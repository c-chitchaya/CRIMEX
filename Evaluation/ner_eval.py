...
#         print('true = ', len(self.true))
#         print(self.true) 
#         print('\n---------------------------\n')
#         print('pred = ', len(self.pred))
#         print(self.pred)
        count = 0
        ...
        for true_ents, pred_ents in zip(self.true, self.pred):
            ...
            if len(true_ents) != len(pred_ents):
                print(count)
                print("true ents: ", len(true_ents))
                #print(true_ents)
                print("\n\npred ents: ", len(pred_ents))
                #print(pred_ents)
                #raise ValueError("Prediction length does not match true example length")
                
                if len(true_ents) > len(pred_ents):
                    add = len(true_ents) - len(pred_ents)
                    for i in range(0,add):
                        pred_ents.append('pad')
                else:
                    add = len(pred_ents) - len(true_ents)
                    for i in range(0,add):
                        true_ents.append('pad')
...
def compute_metrics(true_named_entities, pred_named_entities, tags):
    ...
    eval_metrics = {'correct': 0, 'incorrect': 0, 'partial': 0, 'missed': 0, 'spurious': 0, 'precision': 0, 'recall': 0, 'f1-score':0}
...
def compute_precision_recall(results, partial_or_type=False):
    ...
    results["f1-score"]  = 0 if precision + recall == 0 else 2 * (precision * recall) / (precision + recall)
